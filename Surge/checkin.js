...
$tool.get('https://dict.youdao.com/infoline/style/cardList?mode=publish&client=mobile&style=daily&size=2', function (error, response, data) {
    ...
    let subtitle = '';
    let content = 'shortcuts://run-shortcut?name=Check%20in''dingtalk://dingtalkclient/page/link?url=http://122.226.181.104:12208/H5/index/Index.htm?OP=phone_html5&r=20200306031619356';
    if (!error) {
        if (obj && obj.length > 1) {
            let yi = obj[1];
            content = yi.title + '\n' + yi.summary + '\n\n' + content;
